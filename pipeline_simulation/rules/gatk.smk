...
rule gatk_haplotypecaller:
    ...
    shell:
        ...
        "GenomeAnalysisTK {params.java} -T HaplotypeCaller -R {input.reference} -I {input.bamfile} -o {output.vcf} {params.settings} -L {input.bed} > {log.gatk} 2>&1;"
...
rule gatk_joint_calling:
    ...
    run:
        cmd = "source activate "+ params.conda + "; GenomeAnalysisTK "+ params.java + " -T GenotypeGVCFs -R " + input.reference + " -o " + output.joint_vcf + " -L " + input.bed
